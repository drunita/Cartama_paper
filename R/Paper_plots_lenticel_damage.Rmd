...
df$Inc_increase <- df$Inc_21dph-df$Inc_0dph
df$Sev_increase <- df$Sev_21dph-df$Sev_0dph
...
mat <- data.frame(rep(c("2019_m" , "2020_m" , "2020_t" , "2021_t") , 13) , c(rep("Escondida" , 20) , rep("Sinai" , 32)) , sev , inc)
...
p1 <- p1 +  xlab("") + ylab("Number of necrotic spots")
...
ggsave("C:/Users/Dru/Desktop/Cartama_paper/Figure1_new.jpg",plot=plot,dpi = 300,width = 7,height = 7,units = c("in"))
...
###stack plot
#sev <- c(df1$Sev_0dph , df1$Sev_increase)
#inc <- c(df1$Inc_0dph , df1$Inc_increase)
...
p <-  ggplot(mat1, aes(x = factor(Time), y = Severity)) + 
      geom_bar(stat="summary", color="black", positio=position_dodge(), fill= rep(c("gray99","gray95") ,2), fun = mean) +
      stat_summary(fun.data=mean_sdl, fun.args = list(mult=1), geom="errorbar", color="black", width=0.2) 
...
p <- p +  theme(axis.text.x = element_text(face="bold",size=12, angle=0,family = 'serif'),
              axis.text.y = element_text(face="bold",size=12, angle=0,family = 'serif'))
p <- p + theme(strip.text.x = element_text(size = 14,family = 'serif'))
p <- p + scale_color_manual(name = "Harvest",  values =  c("gray88","gray70","gray47","gray0"))
...
```{r severity and incidence in time plot}
 ...
 plot<-grid.arrange(p + labs(tag = 'A'),p1 + labs(tag = 'B'), ncol=1,nrow=2)
ggsave("C:/Users/Dru/Desktop/Cartama_paper/Figure2_new.jpg",plot=plot,dpi = 300,width = 7,height = 7,units = c("in"))
...
farm <- c(df1$Farm , df1$Farm)
time <-factor(c(rep("0dph",length(sev)/2) , rep("21dph",length(sev)/2)))
harvest <- c(df1$Harvest , df1$Harvest)
...
mat_mean<-tapply(mat$Severity , list(mat$Time,mat$Harvest,mat$Farm),mean)
mat_mean1<-tapply(mat$Incidence , list(mat$Time,mat$Harvest,mat$Farm),mean)
mat_se<-tapply(mat$Severity , list(mat$Time,mat$Harvest,mat$Farm),sd)/sqrt(table(mat$Time,mat$Harvest,mat$Farm))
mat_se1<-tapply(mat$Incidence , list(mat$Time,mat$Harvest,mat$Farm),sd)/sqrt(table(mat$Time,mat$Harvest,mat$Farm))
n <- table(mat$Time,mat$Harvest,mat$Farm)

mat1<-data.frame(Farm = c(rep("Escondida" , 8) , rep("Sinai" , 8)), Time = rep(c(rep("0dph",4) , rep("21dph",4)) , 2) ,
                 Harvest = rep(c("2019_m" , "2020_m" , "2020_t" , "2021_t") , 4), 
                 Severity = c(mat_mean[1,,1] , mat_mean[2,,1] , mat_mean[1,,2] , mat_mean[2,,2]),
                 Severity_se = c(mat_se[1,,1] , mat_se[2,,1] , mat_se[1,,2] , mat_se[2,,2]),
                 Incidence = c(mat_mean1[1,,1] , mat_mean1[2,,1] , mat_mean1[1,,2] , mat_mean1[2,,2]),
                 Incidence_se = c(mat_se1[1,,1] , mat_se1[2,,1] , mat_se1[1,,2] , mat_se1[2,,2]))
...
p1 <- p1 + facet_wrap(~ Farm) + theme_classic() 
p1 <- p1 +  xlab("Harvest") + ylab("Number of necrotic spots") + labs(fill='Days postharvest') 
p1 <- p1 + theme(axis.title =element_text(size=14,family = 'serif'))
p1 <- p1 +  theme(axis.text.x = element_text(face="bold",size=12, angle=0,family = 'serif'),
              axis.text.y = element_text(face="bold",size=12, angle=0,family = 'serif'))
p1 <- p1 + theme(strip.text.x = element_text(size = 14,family = 'serif'))
...
```
*Fig 2.* Lenticel damage for avocado cv. Hass fruits collected from the La Escondida and El Sinai farms between 2019 and 2021 at harvest time (0 days post-harvest, 0 dph) and after 21 days of storage at 6C (21 days post-harvest, 21 dph). Shown are the means and standard errors for the severity (A) and incidence (B) of the damage for the two evaluatin points (0 dph and 21 dph). The * denotes statistical differences at a 95% confidence level according to the  generalized linear mixed models, and the letter after the year denotes main (m) and traviesa (t) harvest.
